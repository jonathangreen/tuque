 ...
 * @param int $code
 *   The error code. These are often the HTTP status codes, however less then 100 is defined 
 *   by the class extending HttpConnection, for eample cURL.
 * @param array $response
 *   The array containing: status, headers, and content of the HTTP request causing the error. 
 *   This is only set if there was a HTTP response sent.
 */
...
abstract class HttpConnection {
  ...
  /**
   * The username to connect with. If no username is desired then use NULL. (Default: NULL)
   * @var type string
   */
  ...
  /**
   * TRUE to check the existence of a common name and also verify that it matches 
   * the hostname provided. (Default: TRUE)
   * @var type boolean
   */
  ...
  /**
   * The maximum number of seconds to allow cURL functions to execute. (Default: cURL default)
   * @var type int
   */
  public $timeout = NULL;
  
  /**
   * The number of seconds to wait while trying to connect. Use 0 to wait indefinitely. (Default: 5)
   * @var type 
   */
  ...
  /**
   * If this is set to true, the connection will be recycled, so that cURL will try to use
   * the same connection for multiple requests. If this is set to FALSE a new connection will
   * be used each time.
   * @var type boolean
   */
...
class CurlConnection extends HttpConnection {
   ...
   */
  private function unallocateCurlContext() {
   ...
   */
  private function setupCurlContext($url) {
    if(!$this->curlContext) {
      $this->getCurlContext();
    }
