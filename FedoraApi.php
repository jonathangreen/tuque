...
class FedoraApiA {
   ...
   *   - download: If true, a content-disposition header value "attachment" will
   *     be included in the response, prompting the user to save the datastream
   *     as a file. A content-disposition header value "inline" will be used
   *     otherwise. The filename used in the header is generated by examining
   *     in order: RELS-INT for the relationship fedora-model:downloadFilename,
   *     the datastream label, and the datastream ID. The file extension
   *     (apart from where the filename is specified in RELS-INT) is determined
   *     from the MIMETYPE.
   *
   * @throws RespositoryException
   *
   ...
   */
  public function getDatastreamDissemination($pid, $dsid, $params = array()) {
    $pid = urlencode($pid);
    $dsid = urlencode($dsid);
    $seperator = '?';
    ...
    $this->connection->addParamArray($request, $seperator, $params, 'download');
...
class FedoraApiM {
   ...
   *     object: if object XML is included with the request, the namespace
